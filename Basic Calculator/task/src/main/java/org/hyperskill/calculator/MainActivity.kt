...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val numericListener =
            View.OnClickListener { view -> calculatorViewModel.handleNumeric((view as Button).text.toString()) }
        val operationListener =
            View.OnClickListener { view -> calculatorViewModel.handleOperational((view as Button).text.toString()) }
        ...
        setListener(binding.addButton, operationListener)
        setListener(binding.multiplyButton, operationListener)
        setListener(binding.divideButton, operationListener)
        setListener(binding.equalButton, operationListener)
        setListener(
            binding.button0,
            View.OnClickListener { calculatorViewModel.handleZero(binding.button0.text.toString()) })
        setListener(
            binding.subtractButton,
            View.OnClickListener { calculatorViewModel.handleSubtraction(binding.subtractButton.text.toString()) })
        setListener(
            binding.dotButton,
            View.OnClickListener { calculatorViewModel.handleDot(binding.dotButton.text.toString()) }) // setOnClickListener is for specific button, while ViewOnClickListener is universal and awaits when this listener will be used
